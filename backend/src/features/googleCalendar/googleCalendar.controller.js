...
const googleCalendarOAuth2Callback = asyncHandler(async (req, res) => {
  ...
  if (!code) {
    ...
    return res.status(400).redirect('/profile?google-auth=error&message=MissingAuthorizationCode');
  }
  ...
  try {
    ...
    if (!user) {
        return res.status(400).redirect('/profile?google-auth=error&message=UserNotFoundForState');
    }

    user.googleAccessToken = tokens.access_token;
    if (tokens.refresh_token) { // Refresh token is not always sent on subsequent authorizations
    ...
    res.status(500).redirect('/profile?google-auth=error&message=TokenExchangeFailed');
  }
});
...
const getUserAuthedClient = async (userId) => {
    const user = await User.findById(userId).select('+googleAccessToken +googleRefreshToken +googleTokenExpiryDate');
    if (!user || !user.isGoogleCalendarAuthorized || !user.googleAccessToken) {
        const err = new Error("User not authorized with Google Calendar or tokens missing.");
    ...
    oauth2Client.setCredentials({
        ...
        expiry_date: user.googleTokenExpiryDate ? user.googleTokenExpiryDate.getTime() : null,
    });
    ...
    if (oauth2Client.isTokenExpiring()) {
        try {
            ...
            await user.save();
            const err = new Error("Failed to refresh Google token. Please re-authorize.");
...
const listAppEvents = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.status(error.statusCode || 500).json({ message: error.message || "Failed to fetch calendar events." });
    }
});
...
const createAppEvent = asyncHandler(async (req, res) => {
    const { summary, description, startTime, endTime, location, attendees, notificationMinutes } = req.body; // attendees as array of emails

    if (!summary || !startTime || !endTime) {
        res.status(400);
        throw new Error("Summary, startTime, and endTime are required for creating an event.");
    }
    ...
    try {
        ...
        res.status(error.statusCode || 500).json({ message: error.message || "Failed to create calendar event." });
    }
});
...
const updateAppEvent = asyncHandler(async (req, res) => {
    ...
    const { summary, description, startTime, endTime, location, attendees, notificationMinutes } = req.body;
    ...
    try {
        ...
        if (!existingEvent.data.description || !existingEvent.data.description.includes(APP_TAG)) {
             if(!existingEvent.data.summary || !existingEvent.data.summary.includes(APP_TAG)){
                res.status(403);
                throw new Error("Event not managed by this application or tag missing.");
             }
        }
        ...
        const eventDetails = {
            ...
            description: description !== undefined ? `${description}\n\nEvent managed by ${APP_TAG}.` : existingEvent.data.description,
            start: startTime ? { dateTime: startTime, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone } : existingEvent.data.start,
            end: endTime ? { dateTime: endTime, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone } : existingEvent.data.end,
        ...
        console.error('Error updating calendar event:', error.response ? error.response.data : error);
        res.status(error.statusCode || 500).json({ message: error.message || "Failed to update calendar event." });
    }
});
...
const getGoogleCalendarAuthStatus = asyncHandler(async (req, res) => {
    ...
    res.json({ isGoogleCalendarAuthorized: user.isGoogleCalendarAuthorized || false });
});
