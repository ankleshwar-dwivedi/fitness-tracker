...
if (!config.mongoURI) {
  console.error("FATAL ERROR: MONGO_URI is not defined in .env. Application cannot start.");
  process.exit(1);
}
if (!config.jwtSecret || config.jwtSecret.length < 32) {
  console.error("FATAL ERROR: JWT_SECRET is not defined or is too short (min 32 chars) in .env. Application cannot start.");
  process.exit(1);
}
if (!config.frontendUrl) {
    console.warn("WARNING: FRONTEND_URL is not defined in .env. Defaulting to http://localhost:5173. This is important for redirects.");
}

// Validate Google Sign-In configurations (make these fatal if this feature is critical)
if (config.nodeEnv !== 'test') {
    if (!config.googleWebClientId) {
        console.error("FATAL ERROR: GOOGLE_WEB_CLIENT_ID for Sign-In is not defined in .env.");
        process.exit(1);
    }
    if (!config.googleWebClientSecret) { // Secret is needed for server-side code exchange
        console.error("FATAL ERROR: GOOGLE_WEB_CLIENT_SECRET for Sign-In is not defined in .env.");
        process.exit(1);
    }
    if (!config.googleSignInRedirectUri) {
        console.error("FATAL ERROR: GOOGLE_SIGN_IN_REDIRECT_URI is not defined in .env.");
        process.exit(1);
    }

    // Warnings for Calendar config (can be errors if calendar is immediately critical)
    if (!config.googleCalendarClientId || !config.googleCalendarClientSecret || !config.googleCalendarRedirectUri) {
        console.warn("WARNING: Google Calendar API credentials (Client ID, Secret, or Redirect URI) are not fully defined. Calendar integration may not work.");
    }
}
