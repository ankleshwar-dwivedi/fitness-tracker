...
const ChatbotDialog = ({ limitedAccess }) => {
  ...
  const callChatbotApi = useCallback(async (payload) => {
    setIsLoading(true);
    setError('');
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || "Chatbot connection error. Please try again.";
  ...
  const handleOptionClick = (optionText) => {
    ...
    callChatbotApi({ currentState: currentChatState, selectedOption: optionText });
  };
  ...
  const toggleChatOpen = () => {
    setIsOpen(!isOpen);
    if (!isOpen && messages.length === 0 && !currentChatState) { // If opening for the first time
        fetchInitialMessage();
    } else if (!isOpen && currentChatState) { // Re-opening, maybe refresh or just show
  ...
  return (
    <div className="fixed bottom-5 right-5 w-80 sm:w-96 h-[30rem] sm:h-[32rem] bg-white rounded-xl shadow-2xl flex flex-col z-[100] border border-gray-200 overflow-hidden">
      <div className="bg-indigo-600 text-white p-3 flex justify-between items-center flex-shrink-0">
        ...
        <button onClick={toggleChatOpen} className="text-indigo-100 hover:text-white p-1 rounded-full">
      ...
      <div className="flex-grow p-3 overflow-y-auto space-y-2.5 text-sm scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        {messages.map((msg) => (
          <div key={msg.timestamp} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[80%] p-2.5 rounded-lg shadow-sm ${
                msg.sender === 'user' ? 'bg-indigo-500 text-white rounded-br-none' :
                msg.sender === 'error' ? 'bg-red-100 text-red-700 border border-red-200 rounded-bl-none' :
        ...
        {isLoading && <div className="text-center text-xs text-gray-400 py-2">FitTrack is thinking...</div>}
        {/* Error state is now part of messages stream */}
        <div ref={messagesEndRef} />
      </div>

      {expectsUserInput && !isLoading && (
        <form onSubmit={handleUserInputSubmit} className="p-2.5 border-t border-gray-200 bg-gray-50 flex-shrink-0">
          <Input
      ...
      {!expectsUserInput && !isLoading && options.length > 0 && (
       ...
       {error && !isLoading && ( // Show general error if options/input not available
         <div className="p-2.5 border-t border-gray-200 text-xs text-red-500 bg-red-50 text-center flex-shrink-0">
            {error} <Button variant="secondary" size="sm" onClick={fetchInitialMessage} className="ml-2 py-0.5 px-1.5 text-xs">Retry</Button>
         </div>
       )}
    </div>
  );
